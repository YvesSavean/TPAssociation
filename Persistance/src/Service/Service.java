...
public class Service {
	...
	public void ajoutArticleCommande(String codeArticle, String login){
		...
		List<Commande> listeDeSesCommandes = cem.chercherCommandeDunAdherent(adh);
		//Si il a déjà passé des commandes
		if(listeDeSesCommandes != null){
			//on récupère la commande managed
			//le premier de la liste, car plusieurs commandes ne sont pas gérées dans le projet Dynamique
			...
			cmd.setId(nombreAleatoire); //problème pour l'id : je veux bien laisser jpa le gerer mais je ne sais pas cmt je le recupère après
		...
		laLigne.setQuantiteArticle(1); //1 par défaut car non géré dans le projet Dynamique
		...
		if(listeDeSesCommandes != null){
			...
			//Je ferme les EntityManagers pour la commande et commandeArticle pour être au même stade de l'autre cas
			cem.close();
			caem.close();
		}else{
			//Si il n'a pas déjà passé de commande
			//Je ferme les EntityManagers car ils vont être réouvert dans la méthode Ajout
			cem.close();
			caem.close();
	...
	//Supression de toutes les commandes (puis des commandes_articles liés) ou le login est égal à celui en paramètre
	public void viderCommande(String login){
		//Les différents EntityManagers
		AdherentEntityManager aem = new AdherentEntityManager();
		CommandeEntityManager cem = new CommandeEntityManager();
		//Pour trouver les commandes d'un adhérent
		List<Commande> listeDeSesCommandes = cem.chercherCommandeDunAdherent(aem.trouver(login));
		//Je ferme les EntityManagers car ils vont être réouvert dans la méthode supprimer
		cem.close();
		aem.close();
	...
	//Liste de tous les codes articles des commandes_articles liés aux commandes où le login est égal à celui en paramètre
	public List<String> listeCommande(String login){
		...
		for (Commande c : listeDeSesCommandes) {
			...
			List<CommandeArticle> listeContenu = caem.chercherCommandeArticleAdh(c);
